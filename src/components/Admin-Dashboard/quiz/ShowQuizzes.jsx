...
export default function ShowQuizzes() {
  ...
  function handleDeleteQuiz(id, video_id) {
            ...
            {quizzes?.map((quiz) => (
              <tr key={quiz.id}>
                ...
                <td className='table-td flex gap-x-2'>
                  <button
                    onClick={() => handleDeleteQuiz(quiz.id, quiz.video_id)}
                  >
                    <svg
                      fill='none'
                      viewBox='0 0 24 24'
  ...
  if (isSuccess && quizzes.length === 0) {
    content = <h3>no quiz available. make some by clicking add button</h3>;
  }
  if (isError) {
    content = <Error message={'Error fetching quizzes'} refetch={refetch} />;
  }
  return (
    <>
      <AdminNavbar />
