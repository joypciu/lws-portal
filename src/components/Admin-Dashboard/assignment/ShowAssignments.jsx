...
export default function ShowAssignments() {
            ...
            {assignments.map((assignment) => (
              <tr key={assignment.id}>
                ...
                <td className='table-td flex gap-x-2'>
                  <button
                    onClick={() => deleteAssignment(Number(assignment.id))}
                  >
                    <svg
                      fill='none'
                      viewBox='0 0 24 24'
                  ...
                  <Link to={`/admin/assignments/edit/${assignment.id}`}>
                    <svg
                      fill='none'
                      viewBox='0 0 24 24'
                      strokeWidth='1.5'
  ...
  if (isSuccess && assignments.length === 0) {
    content = (
      <h3>No assignment is created. add assignement by clicking add button</h3>
    );
  }
  if (isError) {
    content = (
      <Error message={'Error fetching assignments'} refetch={refetch} />
    );
  }
  return (
    <>
          ...
          <div className='px-3 py-20 bg-opacity-10'>
            <div className='w-full flex'>
              <button className='btn ml-auto'>
                <Link to='/admin/add-assignment'>Add Assignment</Link>
              </button>
            </div>
            {content}
          </div>
